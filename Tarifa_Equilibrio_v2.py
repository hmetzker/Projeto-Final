...
# totais do ano atípico e do ano de referência (oferta/ demanda de pax de acordo com o planejamento)
    def soma_excel(self):
        self.total_ano_anterior_km = d_fr2['km coberto'].sum()
        self.total_ano_atipico_km = d_fr1['km coberto'].sum()
        self.total_ano_anterior_pax = d_fr2['pax pagantes'].sum()
        self.total_ano_atipico_pax = d_fr1['pax pagantes'].sum()
...
# mostrando gráfico de barras
        concData = pd.DataFrame([np.float64(tarifa_vig), np.float64(t_eq[2])])
        concData[0].plot.barh()
        plt.show()
